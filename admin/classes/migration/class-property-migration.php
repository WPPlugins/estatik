...
class Es_Property_Migration
{
    ...
    public static function migrate_property( $id )
    {
            ...
            foreach ( $images as $image ) {
                ...
                if ( ! empty( $image ) && file_exists( $upload_dir['basedir'] . $image ) ) {
                    ...
                    if ( ! is_wp_error( $file['tmp_name'] ) ) {
                        ...
                        self::get_logger()->set_message( __( 'Property ' . $post->post_title . ' image: ' . $file['tmp_name']->get_error_message() ), 'warning' );
                    }
                }
            }
        ...
        foreach ( $rel_data_many as $value ) {
            ...
            $term_col = $wpdb->get_col( "SELECT {$value}_id FROM " . $wpdb->prefix . "estatik_properties_{$value}s WHERE prop_id='$id'" );

            if ( $term_col ) {
                $tax = $value == 'appliance' ? 'es_amenities' : 'es_' . $value;

                foreach ( $term_col as $term_id ) {
                    $title = self::get_property_related_data( $term_id, $wpdb->prefix . 'estatik_manager_' . $value . 's', $value . '_title', $value . '_id' );
        ...
        self::get_logger()->set_message( __( 'Property ' . $post->post_title . ' successfully migrated' ), 'success' );
